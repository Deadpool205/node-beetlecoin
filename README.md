# A Node.js beetlecoin Client!


            


node-beetlecoin is a beetlecoin client for Node.js. It is a fork of the excellent Kapitalize Bitcoin Client (now removed from GitHub) intended for use with beetlecoin. The purpose of this repository is:

* Provide a one-stop resource for the Node.js developer to get started with beetlecoin integration.
* Promote Node.js development of beetlecoin web apps.
* Identify and address any incompatibilities with the beetlecoin and Bitcoin APIs that exist now and/or in the future.

## Dependencies

You'll need a running instance of a [beetlecoincoind](https://github.com/beetlecoinblockchain/beetlecoin-core.git) to connect with. 

Then, install the node-beetlecoin NPM package.

`npm install node-beetlecoin`

or

`npm install git://github.com/Deadpool205/node-beetlecoin/`
...
```js
var beetlecoin = require('node-beetlecoin')()

beetlecoin.auth('myusername', 'mypassword')

beetlecoin.getDifficulty(function() {
...
```js
beetlecoin.getNewAddress(function(err, address) {
...
```js
beetlecoin.exec('getNewAddress')

beetlecoin.exec('getbalance', function(err, balance) {
...
All [beetlecoin API](http://api.beetle-coin.com) commands are supported, in lowercase or camelcase form.
...
<td> <b>Currently only available on Mainnet</b> Add a nrequired-to-sign multisignature address to the wallet. Each key is a beetlecoin address or hex-encoded public key. If [account] is specified, assign address to [account]. </td>
<td> N
</td></tr>
<tr>
<td> backupwallet </td>
...
<tr>
<td> dumpprivkey </td>
<td> [beetlecoinaddress] </td>
<td> Reveals the private key corresponding to <beetlecoinaddress< </td>
<td> Y
</td></tr>
<tr>
<td> encryptwallet </td>
...
<td> [account] </td>
<td> Returns the current beetlecoin address for receiving payments to this account. </td>
<td> N
</td></tr>
<tr>
...
<td> getgenerate </td>
<td> </td>
<td> Returns true or false whether beetlecoind is currently generating hashes </td>
<td> N
</td></tr>
<tr>
...
<td> getnewaddress </td>
<td> [account] </td>
<td> Returns a new beetlecoin address for receiving payments.  If [account] is specified (recommended), it is added to the address book so payments received with the address will be credited to [account]. </td>
<td> N
</td></tr>
<tr>
...
<td> [beetlecoinaddress] [minconf=1] </td>
<td> Returns the total amount received by <beetlecoinaddress< in transactions with at least [minconf] confirmations. While some might consider this obvious, value reported by this only considers *receiving* transactions. It does not check payments that have been made *from* this address. In other words, this is not "getaddressbalance". Works only for addresses in the local wallet, external addresses will always show 0. </td>
<td> N
</td></tr>
<tr>
...
<p>To get a list of accounts on the system, execute beetlecoind listreceivedbyaddress 0 true
</p>
</td>
<td> N
</td></tr>
...
<tr>
<td> sendfrom </td>
<td> [fromaccount] [tobeetlecoinaddress] [amount] [minconf=1] [comment] [comment-to] </td>
...
<tr>
<td> signmessage </td>
<td> [beetlecoinaddress] [message] </td>
...
<td> stop </td>
<td> </td>
<td> Stop beetlecoin server. </td>
<td> N
</td></tr>
<tr>
<td> validateaddress </td>
<td> [beetlecoinaddress] </td>
<td> Return information about [beetlecoinaddress]. </td>
<td> N
</td></tr>
<tr>
<td> verifymessage </td>
<td> [beetlecoinaddress] [signature] [message] </td>
...
var beetlecoin = require('beetlecoin')({
    user:'user'
})

beetlecoin.set('pass', 'somn')
beetlecoin.set({port:7142})
...
You may pass an optional function `passphrasecallback` to the `node-beetlecoin` initialization function to manage wallet unlocks. `passphrasecallback` should be a function accepting three arguments:
...
By default `beetlecoind` exposes its JSON-RPC interface via HTTP; that is, all RPC commands are transmitted in plain text across the network! To secure the JSON-RPC channel you can supply `beetlecoind` with a self-signed SSL certificate and an associated private key to enable HTTPS. For example, in your `beetlecoin.conf`:

    rpcssl=1
    rpcsslcertificatechainfile=/etc/ssl/certs/beetlecoind.crt
    rpcsslprivatekeyfile=/etc/ssl/private/beetlecoind.pem

In order to securely access an SSL encrypted JSON-RPC interface you need a copy of the self-signed certificate from the server: in this case `beetlecoind.crt`. Pass your self-signed certificate in the `ca` option and set `https: true` and node-beetlecoin is secured!
    
```js
var fs = require('fs')

var ca = fs.readFileSync('beetlecoind.crt')
